...
entity Rs232RefComp is
    Port ( 
		...
		RDA	: inout std_logic	:= '0';					--Read Data Available(1 quando il dato è disponibile nel registro rdReg)
...
begin
	...
	DBOUT <= rdReg; --uscita parallela che collego a rdReg (buffer di ricezione)
	tfReg <= DBIN;  --(buffer di trasimmsione)
...
--  ----------------------------------------------------------------------------------------
				...
				-- nelle slide c'erano waitFor0 e waitFor1 che sono superflui nel codice
	...
	process (rClk, RST, RD, CE)
		begin
			if RD = '1' or RST = '1' then
				...
				if CE = '1' then
					FE <= frameError;
					OE <= RDA;   --se RDA era ancora alto vuol dire che nessuno ha ancora letto il dato e quindi si avrà overrun (RDA del dato precedente!)
					RDA <= '1';	 --segnala la fine della ricezione (RDA attuale lo metti a 1)
					PE <= parError;
	...
	process (stbeCur, CLK, WR, DBIN, load)
		begin
			...
			case stbeCur is
				...
				when stbeSetTBE =>
					TBE <= '0';  --il write mi fa abbassare il TBE
					if load = '1' then
